...
try:
    ...
    status_bar(APP_VERSION, utc_time(market["updated_at"]), "Personal Intelligence Layer")
    st.markdown(
    ...
    st.info(
        "Tier 1 holdings receive the highest attention weighting, Tier 2 watchlist projects receive "
        "active monitoring, and Tier 3 provides mainstream Australian-market context. Edit "
        "`personal_market_config.py` to add, remove or reprioritise projects."
    )
    st.caption(
        "Attention is based on price momentum, turnover relative to market capitalisation, conviction "
        "and personal priority. It is a monitoring signal, not investment advice."
    )
