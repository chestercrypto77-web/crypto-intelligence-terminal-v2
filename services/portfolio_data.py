...
def build_portfolio_snapshot(coins: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    high_risk_weight = sum(
        float(row["weight"]) for row in rows if row["risk"].upper() == "HIGH"
    )

    normalised_change = None
    if available_change_weight:
        normalised_change = weighted_change * 100.0 / available_change_weight

    overall_risk = "HIGH" if high_risk_weight >= 20 else "MEDIUM"
